...
static void create_http_header(char * dest, const char * method, const char * uri, const char * content_type, const char * host_ip){
    static char printer_token_str[CONNECT_TOKEN_SIZE + 1]; // extra space of end of line
    char header_gen_part[MAX_HEADER_GEN_PART_SIZE + 1]; // 32 Content type + 24 Host
    variant8_t printer_token = eeprom_get_var(EEVAR_CONNECT_TOKEN);
    strlcpy(printer_token_str, printer_token.pch, CONNECT_TOKEN_SIZE + 1);
    variant8_done(&printer_token);
    ...
    snprintf(dest, HEADER_MAX_SIZE, "%s %s HTTP/1.0\r\nPrinter-Token: %s\r\n%s\r\n", method, uri, printer_token_str, header_gen_part);
}
